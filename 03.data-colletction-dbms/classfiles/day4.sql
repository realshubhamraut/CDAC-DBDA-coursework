...
CREATE table if not exists countires(c_id int, c_name varchar(200));


select * from countries;

create table country_temp_data
AS select * FROM countries;

select * from country_temp_data;
...
add column depart_name varchar(30) default 'Development';
...
alter table emp_ex add column emp_p int(10) auto_increment primary key;
...
alter table emp_ex2 add constraint pr_key primary key(emp_id);
desc emp_ex2;

alter table emp_ex2 add constraint chk_sal check (salary>50000);
...
-- checking constraint and alias

SELECT CONSTRAINT_NAME, TABLE_NAME, CONSTRAINT_TYPE
FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
WHERE TABLE_NAME = 'emp_ex2';
...
alter table attendance add primary key (stu_id);
ALTER TABLE attendance ADD CONSTRAINT chk_75 CHECK (days_present > total_working_days * 0.75);  -- here we added the constraint to check if the attenace is minimum 75%
alter table attendance add column address varchar(200) default 'Mohol';
...
INSERT INTO attendance (stu_id, std_name, days_present, total_working_days, contact_no) 
VALUES (1, 'shubham', 89, 100, 9878899876);
...
alter table employees add column doj datetime;

alter table employees modify column doj timestamp default current_timestamp;
alter table employees add column status enum('Active','Inactive', 'Resign','long leave') default 'long leave';

alter table emp_ex2 modify column emp_id int(10) auto_increment, auto_increment=2000 OFFSET 2000;
